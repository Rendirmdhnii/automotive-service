...
export default function Towing() {
  ...
  return (
    <section
      id="towing"
      className="relative py-24 bg-gradient-to-br from-red-50/40 via-white to-red-50/20 overflow-hidden border-y border-brand-red/10"
      ref={sectionRef}
    >
      ...
      {/* Red Ambient Glows in Light Theme */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-4xl h-96 rounded-full bg-brand-red/[0.04] blur-[150px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          ...
          >
            {/* Live Indicator */}
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-brand-red/35 bg-brand-red/5 backdrop-blur-md mb-6 w-max shadow-[0_4px_15px_rgba(220,38,38,0.08)]">
              <span className="w-2.5 h-2.5 rounded-full bg-brand-red animate-ping" />
              <span className="font-poppins text-xs font-bold uppercase tracking-[0.2em] text-brand-red">
                LAYANAN DARURAT 24 JAM
              </span>
            </div>

            <h2 className="font-poppins font-black text-4xl sm:text-5xl lg:text-6xl tracking-tight leading-none text-gray-900 mb-6">
              Butuh Derek Cepat? <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-red to-brand-red-light text-glow-red">
            ...
            <div className="grid grid-cols-3 gap-4 mb-8">
              <div className="flex flex-col p-4 rounded-xl bg-white border border-black/5 text-center shadow-sm">
                <Clock className="w-5 h-5 text-brand-red mx-auto mb-2" />
                <span className="font-poppins font-bold text-xs text-gray-800">Respon Cepat</span>
              </div>
              <div className="flex flex-col p-4 rounded-xl bg-white border border-black/5 text-center shadow-sm">
                <MapPin className="w-5 h-5 text-brand-red mx-auto mb-2" />
                <span className="font-poppins font-bold text-xs text-gray-800">Area Sidoarjo</span>
              </div>
              <div className="flex flex-col p-4 rounded-xl bg-white border border-black/5 text-center shadow-sm">
                <ShieldCheck className="w-5 h-5 text-brand-red mx-auto mb-2" />
                <span className="font-poppins font-bold text-xs text-gray-800">Garansi Aman</span>
              </div>
            </div>

            {/* Towing Cases Bullet Checklist */}
            <h3 className="font-poppins font-bold text-sm uppercase tracking-[0.15em] text-gray-800 mb-4">
              Kasus Penyelamatan Towing:
            </h3>
            <div className="space-y-3.5 mb-10">
              {TOWING_CASES.map((tcase, idx) => (
                <div key={idx} className="flex items-start gap-3">
                  <div className="mt-1 flex-shrink-0 w-5 h-5 rounded-full bg-brand-red/10 border border-brand-red/20 flex items-center justify-center">
                    <span className="w-1.5 h-1.5 rounded-full bg-brand-red" />
                  </div>
                  <div>
                    <h4 className="font-poppins font-bold text-sm text-gray-900">{tcase.title}</h4>
                    <p className="font-inter text-xs text-gray-500 mt-0.5">{tcase.desc}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          >
            ...
            <div className="absolute -inset-4 rounded-3xl border border-brand-red/10 pointer-events-none scale-102" />
            
            {/* Warning indicator decoration */}
            <div className="absolute -top-3 -right-3 w-10 h-10 rounded-full bg-brand-red flex items-center justify-center animate-bounce shadow-[0_4px_15px_rgba(220,38,38,0.3)]">
            ...
            <div className="relative rounded-2xl overflow-hidden shadow-[0_15px_40px_rgba(0,0,0,0.1)] border border-brand-red/20 group aspect-[4/3] sm:aspect-video lg:aspect-[3/4]">
              ...
              <div className="absolute bottom-6 left-6 right-6 p-4 rounded-xl glass-card-no-hover border border-brand-red/20 shadow-md">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    <span className="font-poppins font-bold text-xs text-gray-900">Armada Gendong (Flatbed)</span>
                    <span className="font-inter text-[10px] text-gray-600">Siap meluncur dalam hitungan menit</span>
                  </div>
                </div>
              </div>
            </div>
